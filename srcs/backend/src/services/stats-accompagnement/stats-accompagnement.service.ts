...
export class StatsAccompagnementService {
  ...
  async getMainData(date_debut, date_fin) {
    ...
    let tmp_nb_cand = await cdpenrcand
      .aggregate([
        {
          $match: {
      ...
      .toArray();
    let tmp_nb_suivi = await cdpsuivi
      .aggregate([
        {
      ...
      .toArray();
    let tmp_nb_sortie_positive = await cdpsuivi
      .aggregate([
        {
          $match: {
    ...
    let tmp_nb_cand_semaine = await cdpenrcand
      .aggregate([
        {
          $match: {
      ...
      .toArray();
    let tmp_remplissage_semaine = await cdpenrcand
      .aggregate([
        {
          $match: {
            date_atelier: { $gte: startOfWeek, $lt: endOfWeek },
    ...
    let nb_asso_prescr_semaine = await cdpenrcand
      .aggregate([
        {
          $match: {
            date_atelier: { $gte: startOfWeek, $lt: endOfWeek },
    ...
    result.right.structure_prescr =
      nb_asso_prescr_semaine[0]?.uniqueStructuresCount || 0;

    return result;
  }

  async get_nb_passage_cdp(cdpenrcand, date_debut, date_fin) {
    const nb_passage_cdp = await cdpenrcand
      .aggregate([
        {
          $match: {
            date_atelier: { $gte: date_debut, $lt: date_fin },
  ...
  async get_nb_absents(cdpenrcand, date_debut, date_fin) {
    const nb_absents_cdp = await cdpenrcand
      .aggregate([
        {
          $match: {
            date_atelier: { $gte: date_debut, $lt: date_fin },
  ...
  async get_nb_suivis(cdpsuivi, date_debut, date_fin) {
    let tmp_nb_suivis = await cdpsuivi
      .aggregate([
        {
          $sort: { date_suivi: -1 },
        },
  ...
  async get_nb_prescription_cdp_attente(cdpenrcand, date_debut, date_fin) {
    const nb_prescription_cdp_attente = await cdpenrcand
      .aggregate([
        {
          $match: {
  ...
  async get_nb_prescription_cdp(cdpenrcand, date_debut, date_fin) {
    const nb_prescription_cdp = await cdpenrcand
      .aggregate([
        {
          $match: {
            date_creation: { $gte: date_debut, $lt: date_fin },
  ...
  async get_top_structures_par_statut(cdpenrcand, date_debut, date_fin) {
    const top_structures_par_statut = await cdpenrcand
      .aggregate([
        {
          $match: {
            date_creation: { $gte: date_debut, $lt: date_fin },
            statut: {
              $in: ['Présent', 'Absent', 'Positionné', 'A Positionner'],
            },
        ...
        },
        {
          $group: {
            ...
            topStructures: {
              $push: { structure_id: '$_id.structure_id', count: '$count' },
            },
          },
        },
        {
  ...
  async get_resultats_structures(cdpenrcand, date_debut, date_fin) {
    const resultats_structures = await cdpenrcand
      .aggregate([
        {
          $match: {
            date_atelier: { $gte: date_debut, $lt: date_fin },
  ...
  private genererResumeStructures(resultats: any): string {
    const { meilleures, moins_bonnes } = resultats[0] || {
      meilleures: [],
      moins_bonnes: [],
    };
    ...
    const formatListe = (liste: any[], titre: string): string => {
      ...
      liste.forEach((structure, index) => {
        texte +=
          `  ${index + 1}. ${structure.structure_id} - ` +
    ...
    const meilleuresStr = formatListe(
      meilleures,
      '🏆 Top 3 des structures avec le meilleur taux de présence',
    );
    const moinsBonnesStr = formatListe(
      moins_bonnes,
      '📉 Top 3 des structures avec le plus faible taux de présence',
    );
  ...
  async getViewData(date_debut, date_fin) {
    ...
    const array_prescri_by_month = await this.getPrescriByMonth(
      cdpenrcand,
      date_debut,
      date_fin,
    );

    const nb_passage_cdp = await this.get_nb_passage_cdp(
      cdpenrcand,
      date_debut,
      date_fin,
    );
    const nb_passage_fixe =
      nb_passage_cdp.find((item) => item.type_atelier === 'CDP FIXE')?.count ||
      0;
    const nb_passage_mobile =
      nb_passage_cdp.find((item) => item.type_atelier === 'CDP MOBILE')
        ?.count || 0;

    const nb_absents_cdp = await this.get_nb_absents(
      cdpenrcand,
      date_debut,
      date_fin,
    );

    const nb_passage_qpv = await this.get_nb_qpv(
      cdpenrcand,
      date_debut,
      date_fin,
    );
    const nb_passage_qpv_fixe =
      nb_passage_qpv.find((item) => item._id === 'CDP FIXE')?.count || 0;
    const nb_passage_qpv_mobile =
      nb_passage_qpv.find((item) => item._id === 'CDP MOBILE')?.count || 0;

    let tmp_nb_sortie_positive = await this.get_nb_sortie_positive(
      cdpsuivi,
      date_debut,
      date_fin,
    );

    let tmp_nb_suivis = await this.get_nb_suivis(
      cdpsuivi,
      date_debut,
      date_fin,
    );

    let note_satisfaction = await this.get_note_satisfaction(
      cdpenrcand,
      date_debut,
      date_fin,
    );

    const nb_prescription_cdp_attente =
      await this.get_nb_prescription_cdp_attente(
        cdpenrcand,
        date_debut,
        date_fin,
      );

    const presc_fixe_attente =
      nb_prescription_cdp_attente.find((r) => r.type_atelier === 'CDP FIXE')
        ?.count || 0;
    const prescr_mobile_attente =
      nb_prescription_cdp_attente.find((r) => r.type_atelier === 'CDP MOBILE')
        ?.count || 0;
    const total_prescr_attente =
      presc_fixe_attente +
      prescr_mobile_attente +
      (nb_prescription_cdp_attente.find(
        (r) => r.type_atelier === 'CDP FIXE OU MOBILE',
      )?.count || 0);

    const nb_prescription_cdp = await this.get_nb_prescription_cdp(
      cdpenrcand,
      date_debut,
      date_fin,
    );
    ...
    const resultats_structures = await this.get_resultats_structures(
      cdpenrcand,
      date_debut,
      date_fin,
    );
    const resultat_structure_string =
      this.genererResumeStructures(resultats_structures);

    const nb_cand_at_co = await this.get_nb_cand_at_co(
      atcoenrcand,
      date_debut,
      date_fin,
    );
    const nb_candi_unique_at_co = await this.get_nb_cand_unique_at_co(
      atcoenrcand,
      date_debut,
      date_fin,
    );

    const nb_cand_bien_etre = await this.get_nb_cand_bien_etre(
      bienetreenrcand,
      date_debut,
      date_fin,
    );
    ...
    result.nb_prescr_fixe =
      (resultFormatted['CDP FIXE']?.['Présent'] || 0) +
      (resultFormatted['CDP FIXE']?.['A Positionner'] || 0) +
      (resultFormatted['CDP FIXE']?.['Absent'] || 0) +
      (resultFormatted['CDP FIXE']?.['Positionné'] || 0);
    result.nb_prescr_mobile =
      (resultFormatted['CDP MOBILE']?.['Présent'] || 0) +
      (resultFormatted['CDP MOBILE']?.['A Positionner'] || 0) +
      (resultFormatted['CDP MOBILE']?.['Absent'] || 0) +
      (resultFormatted['CDP MOBILE']?.['Positionné'] || 0);
    result.nb_prescr_fixe_attente = presc_fixe_attente || 0;
    result.nb_prescr_mobile_attente = prescr_mobile_attente || 0;
    result.nb_prescr =
      (resultFormatted['Inconnu']?.['Présent'] || 0) +
      (resultFormatted['Inconnu']?.['A Positionner'] || 0) +
      (resultFormatted['Inconnu']?.['Absent'] || 0) +
      (resultFormatted['Inconnu']?.['Positionné'] || 0);
    ...
    result.nb_cand_cdp_et_bien_etre =
      nb_cand_bien_etre[0]?.cdp_et_bien_etre || 0;
