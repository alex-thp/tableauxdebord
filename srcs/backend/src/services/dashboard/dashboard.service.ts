...
export class DashboardService {
  ...
  isTreeNodeList(
    next: TreeNode[] | ObjectifData[] | undefined,
  ): next is TreeNode[] {
  ...
  transformData(apiData: ApiResponse[]): any {
    ...
    const isTreeNodeList = (
      next:
        | TreeNode[]
        | { objectif: number; realise: number | null }[]
        | undefined,
    ): next is TreeNode[] => {
      return Array.isArray(next) && (next.length === 0 || 'label' in next[0]);
    };
    ...
    for (const entry of apiData) {
      ...
      if (indicateur[0]?.action_localite)
        indicateur[0].action_localite = this.normalizeToArray(
          indicateur[0].action_localite,
        );
      if (indicateur[0]?.sujet_localite)
        indicateur[0].sujet_localite = this.normalizeToArray(
          indicateur[0].sujet_localite,
        );
      if (indicateur[0]?.sujet_critere)
        indicateur[0].sujet_critere = this.normalizeToArray(
          indicateur[0].sujet_critere,
        );
      ...
      for (const actionLocalite of actionLocalites) {
        const localiteNode = getOrCreateNode(
          tree,
          actionLocalite || "n'importe quel département de la région",
        );

        if (!isTreeNodeList(localiteNode.next)) continue;

        for (const sujetLocalite of sujetLocalites) {
          const sujetLocaliteNode = getOrCreateNode(
            localiteNode.next,
            sujetLocalite,
          );
          ...
          const actionNode = getOrCreateNode(
            sujetLocaliteNode.next,
            indicateur[0].action || 'Action inconnue',
          );

          if (!isTreeNodeList(actionNode.next)) continue;

          const sujetNode = getOrCreateNode(
            actionNode.next,
            indicateur[0].sujet || 'Sujet inconnu',
          );
    ...
    function sortCriteriaByDateFin(nodes: TreeNode[]) {
      for (const node of nodes) {
        ...
        if (children.length > 0 && isObjectifNode(children[0])) {
          children.sort((a, b) => {
            const dateA = parseDateFR(
              (a.next![0] as ObjectifData).date_fin,
            ).getTime();
            const dateB = parseDateFR(
              (b.next![0] as ObjectifData).date_fin,
            ).getTime();
            return dateA - dateB;
          });
        } else {
  ...
  async getRapportXIndicateurData(today, database) {
    ...
    const data = await db
      .collection('rapportxindicateurs')
      .aggregate([
        {
          $match: {
            date_debut: { $lt: today },
