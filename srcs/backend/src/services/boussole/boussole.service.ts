...
export class BoussoleService {
  ...
  async getBoussoleData(date_debut, date_fin) {
    ...
    data.accompagnement = await this.getAccompagnementData(
      date_debut,
      date_fin,
    );
  ...
  async getBenevolesData(date_debut: Date, date_fin: Date) {
    ...
    const nb_presence_atelier_benev = await cdpenrbenev
      .aggregate([
        {
          $match: {
            benevole_id: { $ne: null },
    ...
    const nb_presence_atelier_benev_93_95 = await cdpenrbenev
      .aggregate([
        {
          $match: {
            benevole_id: { $ne: null },
    ...
    const repart_benev_nb_atelier = await cdpenrbenev
      .aggregate([
        {
          $lookup: {
            from: 'cdps',
    ...
    const nb_benev_session_accueil = await evenementbenevxbenev
      .aggregate([
        {
          $match: {
            statut: { $eq: 'Présent' },
    ...
    const nb_present_sensi = await evenementbenev
      .aggregate([
        {
          $match: {
    ...
    const nb_evenement_sensi = await evenementbenev
      .aggregate([
        {
          $match: {
            date: { $gte: date_debut, $lt: date_fin },
    ...
    const nb_present_unique_sensi = await evenementbenev
      .aggregate([
        {
          $match: {
            date: { $gte: date_debut, $lt: date_fin },
      ...
      .aggregate([
        ...
        // 5. Grouper par est_be Oui / Non
        {
          $group: {
            _id: {
              est_be: {
                $cond: [{ $eq: ['$dernier_enr.est_be', 'Oui'] }, 'Oui', 'Non'],
              },
            },
            count: { $sum: 1 },
          },
    ...
    const result = await cdpenrbenev
      .aggregate([
        {
          $match: {
            date_atelier: { $gte: date_debut, $lt: date_fin },
        ...
        },
        {
          $project: {
            ...
            pourcentage: {
              $multiply: [
                { $divide: ['$tout_a_fait_pertinent', '$total'] },
                100,
              ],
            },
          },
    ...
    const nombre_benevoles_adherents = await benev
      .aggregate([
        {
          $match: {
            tableau_date_adhesion: {
              $elemMatch: {
                $gte: adhesion_date_debut,
    ...
    const nombre_benevoles_adherents_droit_vote = await benev
      .aggregate([
        {
          $match: {
            tableau_date_adhesion: {
    ...
    const benevoles_actifs = await cdpenrbenev
      .aggregate([
        {
          $match: {
            benevole_id: { $ne: null },
    ...
    data.nb_actions_benevole_mobilisees =
      (await nb_presence_atelier_benev[0]?.count) || 0;
    data.nb_actions_benevole_mobilisees_93_95 =
      (await nb_presence_atelier_benev_93_95[0]?.count) || 0;
    data.nb_ateliers_par_benevole_unique =
      (await repart_benev_nb_atelier.length) > 0
        ? parseFloat(
            (
              data.nb_actions_benevole_mobilisees /
              repart_benev_nb_atelier.reduce((sum, item) => sum + item.count, 0)
            ).toFixed(2),
          )
        : 0; // A checker
    data.nb_venues_ateliers['1x'] =
      (await repart_benev_nb_atelier.find((item) => item.type === '1')
        ?.count) || 0;
    data.nb_venues_ateliers['2x'] =
      (await repart_benev_nb_atelier.find((item) => item.type === '2')
        ?.count) || 0;
    data.nb_venues_ateliers['3x'] =
      (await repart_benev_nb_atelier.find((item) => item.type === '3')
        ?.count) || 0;
    data.nb_venues_ateliers['4x et plus'] =
      (await repart_benev_nb_atelier.find((item) => item.type === '4+')
        ?.count) || 0;
    data.nb_nouveaux_benevoles_session_accueil =
      (await nb_benev_session_accueil[0]?.total) || 0;
    data.taux_transformation_session_accueil =
      (await nb_benev_session_accueil[0]) &&
      nb_benev_session_accueil[0].total > 0
        ? parseFloat(
            (
              (nb_benev_session_accueil[0].avec_date_premier_atelier /
                nb_benev_session_accueil[0].total) *
              100
            ).toFixed(2),
          )
        : 0;
    data.nb_evenements_sensi_visites = await nb_evenement_sensi[0]?.count;
    data.nombre_benevoles_adherents =
      (await nombre_benevoles_adherents[0]?.count) || 0;
    data.nb_participants_uniques_sensi_visites =
      nb_present_unique_sensi[0]?.count || 0;
    data.nombre_benevoles_adherents_droit_vote =
      nombre_benevoles_adherents_droit_vote[0]?.count || 0;
    data.benevoles_actifs = benevoles_actifs.length || 0;
    data.pourcent_benevoles_adherents =
      benevoles_actifs.length > 0
        ? parseFloat(
            (
              (data.nombre_benevoles_adherents / benevoles_actifs.length) *
              100
            ).toFixed(2),
          )
        : 0;
    ...
    const ouiCount =
      (await benevoles.find((r) => r._id.est_be === 'Oui')?.count) || 0;
    const nonCount =
      (await benevoles.find((r) => r._id.est_be === 'Non')?.count) || 0;
    ...
    data.taux_satisfaction_utilite_benevolat =
      result[0] && data.nb_actions_benevole_mobilisees > 0
        ? parseFloat(
            ((result[0].tout_a_fait_pertinent / result[0].total) * 100).toFixed(
              2,
            ),
          )
        : 0;
  ...
  async getPcData(date_debut, date_fin) {
    ...
    let nb_collecte = await evenement_pc
      .aggregate([
        {
          $match: {
            date: { $gte: date_debut, $lt: date_fin },
    ...
    const nbBenevolesUniques = await cdpenrbenev
      .aggregate([
        {
          $match: {
            benevole_id: { $ne: null },
    ...
    const nb_cdp_moyen_be = await cdpenrbenev
      .aggregate([
        {
          $lookup: {
            from: 'cdps',
    ...
    data.nb_collab_3x_plus =
      (await raw.find((item) => item.type === '3+')?.count) || 0;
    data.nb_collab_plus_un_atelier =
      (await (raw.find((item) => item.type === '2')?.count || 0)) +
      data.nb_collab_3x_plus;
    data.nb_cdp_moyen_be = (await nb_cdp_moyen_be[0]?.moyenne_cdp)
      ? parseFloat(nb_cdp_moyen_be[0].moyenne_cdp.toFixed(2))
      : 0;
    return data;
  }
  ...
  async getAccompagnementData(date_debut, date_fin) {
    ...
    const nbCandCdpFixeResult = await cdpenrcand
      .aggregate([
        {
          $match: {
            date_atelier: { $gte: date_debut, $lt: date_fin },
    ...
    const nbCandCdpMobileResult = await cdpenrcand
      .aggregate([
        {
          $match: {
            date_atelier: { $gte: date_debut, $lt: date_fin },
    ...
    const nb_cand_at_co = await atcoenrcand
      .aggregate([
        {
          $match: {
            date_atelier: { $gte: date_debut, $lt: date_fin },
    ...
    const nb_cand_at_co_unique = await atcoenrcand
      .aggregate([
        {
          $match: {
            date_atelier: { $gte: date_debut, $lt: date_fin },
    ...
    const nb_cand_bien_etre = await bienetreenrcand
      .aggregate([
        {
          $match: {
            date_atelier: { $gte: date_debut, $lt: date_fin },
        ...
        },
        {
          $group: {
            ...
            cdp_et_bien_etre: {
              $sum: {
                $cond: [{ $gt: [{ $size: '$present_cdps' }, 0] }, 1, 0],
              },
            },
          },
        },
    ...
    const nb_absent_cdp = await cdpenrcand
      .aggregate([
        {
          $match: {
            date_atelier: { $gte: date_debut, $lt: date_fin },
    ...
    const nb_absent_bien_etre = await bienetreenrcand
      .aggregate([
        {
          $match: {
            date_atelier: { $gte: date_debut, $lt: date_fin },
    ...
    const nb_passage_qpv = await cdpenrcand
      .aggregate([
        {
          $match: {
            date_atelier: { $gte: date_debut, $lt: date_fin },
    ...
    const nb_niveau_lcs = await cdpenrcand
      .aggregate([
        {
          $match: {
            date_atelier: { $gte: date_debut, $lt: date_fin },
    ...
    let note_satisfaction = await cdpenrcand
      .aggregate([
        {
          $match: {
            date_atelier: { $gte: date_debut, $lt: date_fin },
    ...
    let nb_sortie_positive_fin_parcours = await cdpsuivi
      .aggregate([
        {
    ...
    let nb_sortie_positive_remob = await cdpsuivi
      .aggregate([
        {
          $sort: { date_suivi: -1 }, // on trie du plus récent au plus ancien
        },
    ...
    let nb_sortie_negative_fin_parcours = await cdpsuivi
      .aggregate([
        {
          $sort: { date_suivi: -1 }, // on trie du plus récent au plus ancien
        },
    ...
    let nb_sortie_negative_remob = await cdpsuivi
      .aggregate([
        {
          $sort: { date_suivi: -1 }, // on trie du plus récent au plus ancien
        },
    ...
    data.nb_cand_cdp_fixe =
      (await nbCandCdpFixeResult.length) > 0 ? nbCandCdpFixeResult[0].count : 0;
    data.nb_cand_cdp_mobile =
      (await nbCandCdpMobileResult.length) > 0
        ? nbCandCdpMobileResult[0].count
        : 0;
    data.nb_cand_at_co =
      (await nb_cand_at_co.length) > 0 ? nb_cand_at_co[0].total_candidats : 0;
    data.nb_cand_bien_etre =
      (await nb_cand_bien_etre.length) > 0 ? nb_cand_bien_etre[0].total : 0;
    data.nb_cand_cdp_et_at_co =
      (await nb_cand_at_co[0]?.candidats_avec_cdp) || 0;
    data.nb_cand_unique_at_co =
      (await nb_cand_at_co_unique.length) > 0
        ? nb_cand_at_co_unique[0].total_candidats
        : 0;
    data.nb_cand_unique_at_co_avec_cdp =
      (await nb_cand_at_co_unique[0]?.candidats_avec_cdp) || 0;
    data.nb_absent_cdp =
      (await nb_absent_cdp.length) > 0 ? nb_absent_cdp[0].count : 0;
    data.nb_absent_bien_etre =
      (await nb_absent_bien_etre.length) > 0 ? nb_absent_bien_etre[0].count : 0;
    data.nb_cand_qpv_fixe =
      (await nb_passage_qpv.find((item) => item._id === 'CDP FIXE')?.count) ||
      0;
    data.nb_cand_qpv_mobile =
      (await nb_passage_qpv.find((item) => item._id === 'CDP MOBILE')?.count) ||
      0;
    data.nb_fin_parcours =
      (await nb_niveau_lcs.find((item) => item._id === 'FIN DE PARCOURS')
        ?.count) || 0;
    data.nb_remob =
      (await nb_niveau_lcs.find((item) => item._id === 'REMOBILISATION')
        ?.count) || 0;
    data.taux_satisfaction_cdp =
      (await note_satisfaction[0]?.avg_note.toFixed(2)) || 0;
    data.taux_sortie_positive_fin_parcours =
      (await nb_sortie_positive_fin_parcours[0]?.total) || 0;
    data.taux_sortie_positive_remob =
      (await nb_sortie_positive_remob[0]?.total) || 0;
    data.taux_sortie_negative_fin_parcours =
      (await nb_sortie_negative_fin_parcours[0]?.total) || 0;
    data.taux_sortie_negative_remob =
      (await nb_sortie_negative_remob[0]?.total) || 0;
