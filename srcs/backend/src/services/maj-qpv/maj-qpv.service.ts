...
export class MajQpvService {
  ...
  async fetch_data(cdpenrcand) {
    ...
    console.log(
      `code_postal = ${cdpenrcand.candidat_code_postal} nom_commune = ${cdpenrcand.candidat_commune_ville} nom_voie = ${cdpenrcand.candidat_numero_et_rue}`,
    );
    console.log(type_qpv);
    return {
      id: cdpenrcand.record_id,
      fields: {
        QPV_AUTOMATISATION: type_qpv,
        QPV_NOM_QUARTIER_AUTOMATISATION: nom_quartier,
        QPV_CODE_QUARTIER_AUTOMATISATION: code_quartier,
      },
    };
  }
  ...
  async majQPVFunction() {
    ...
    let result;
    try {
      let tabToUpdate: Array<{
        id: string;
        fields: {
          QPV_AUTOMATISATION: string;
          QPV_NOM_QUARTIER_AUTOMATISATION: string;
          QPV_CODE_QUARTIER_AUTOMATISATION: string;
        };
      }> = [];
      let i = 0;
      ...
      const cdp_enr_cand_array = await cdp_enr_cand
        .find({
          statut: { $eq: 'Présent' },
          qpv_automatisation: { $eq: '' },
          $or: [{ qpv_automatisation: { $exists: true, $eq: '' } }],
        })
        .toArray();
      ...
      for (let cdpenrcand of cdp_enr_cand_array) {
        if (i >= 9) {
          await this.update_airtable_10_by_10(
            tabToUpdate,
            process.env.AIRTABLE_CDP_ENR_CAND_TABLE,
          );
          tabToUpdate = [];
          i = 0;
        }
      ...
      for (let bienetreenrcand of bien_etre_enr_cand_array) {
        if (i >= 9) {
          await this.update_airtable_10_by_10(
            tabToUpdate,
            process.env.AIRTABLE_BIEN_ETRE_ENR_CAND_TABLE,
          );
          tabToUpdate = [];
          i = 0;
        }
      ...
      for (let atcoenrcand of at_co_enr_cand_array) {
        if (i >= 9) {
          await this.update_airtable_10_by_10(
            tabToUpdate,
            process.env.AIRTABLE_AT_CO_ENR_CAND_TABLE,
          );
          tabToUpdate = [];
          i = 0;
        }
