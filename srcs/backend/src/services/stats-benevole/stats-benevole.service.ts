...
export class StatsBenevoleService {
  ...
  async get_nb_nouveau_benevole_unique(cdpenrbenev, date_debut, date_fin) {
    const nbBenevolesUniquesAncien = await cdpenrbenev
      .aggregate([
        {
          $match: {
            benevole_id: { $ne: null },
  ...
  async get_nb_actions_benev(evenement_benev, date_debut, date_fin) {
    const nb_actions_benev = await evenement_benev
      .aggregate([
        {
  ...
  async get_nb_benevole_unique_semaine(cdpenrbenev, startOfWeek, endOfWeek) {
    const nbBenevolesUniques_semaine = await cdpenrbenev
      .aggregate([
        {
          $match: {
            benevole_id: { $ne: null },
  ...
  async get_nb_nouveau_benevole_unique_semaine(
    cdpenrbenev,
    startOfWeek,
    endOfWeek,
  ) {
    const nbBenevolesUniquesAncien_semaine = await cdpenrbenev
      .aggregate([
        {
          $match: {
            benevole_id: { $ne: null },
  ...
  async nb_cdp_totaux_semaine(cdpenrbenev, startOfWeek, endOfWeek) {
    const nbCdpsTotaux_semaine = await cdpenrbenev
      .aggregate([
        {
          $match: {
            benevole_id: { $ne: null },
  ...
  async getMainData(date_debut, date_fin) {
    ...
    const nbBenevolesUniques = await this.get_nb_benevoles_unique(
      cdpenrbenev,
      date_debut,
      date_fin,
    );
    const nbBenevolesUniquesAncien = await this.get_nb_nouveau_benevole_unique(
      cdpenrbenev,
      date_debut,
      date_fin,
    );
    const nb_actions_benev = await this.get_nb_actions_benev(
      evenement_benev,
      date_debut,
      date_fin,
    );
    const nbBenevolesUniques_semaine =
      await this.get_nb_benevole_unique_semaine(
        cdpenrbenev,
        startOfWeek,
        endOfWeek,
      );
    const nbBenevolesUniquesAncien_semaine =
      await this.get_nb_nouveau_benevole_unique_semaine(
        cdpenrbenev,
        startOfWeek,
        endOfWeek,
      );
    ...
    const nbcdp_semaine = await this.get_nb_cdp_semaine(
      cdp,
      startOfWeek,
      endOfWeek,
    );
    const nbCdpsTotaux_semaine = await this.nb_cdp_totaux_semaine(
      cdpenrbenev,
      startOfWeek,
      endOfWeek,
    );
    ...
    let remplissage_semaine =
      (nbCdpsTotaux_semaine[0]?.cdpsTotaux / nb_remplissage_semaine) * 100 || 0;
    ...
    result.left.nv_benevole =
      nbBenevolesUniquesAncien[0]?.benevolesUniques || 0;
    result.left.nb_action = nb_actions_benev[0]?.count || 0;

    result.right.nb_benevole =
      nbBenevolesUniques_semaine[0]?.benevolesUniques || 0;
    result.right.nv_benevole =
      nbBenevolesUniquesAncien_semaine[0]?.benevolesUniques || 0;
    result.right.remplissage = remplissage_semaine || 0;
    return result;
  }

  async get_nb_session_acc(evenement_benev, date_debut, date_fin) {
    const nb_session_acc = await evenement_benev
      .aggregate([
        {
          $match: {
  ...
  async get_nb_sensi(evenement_benev, date_debut, date_fin) {
    const raw = await evenement_benev
      .aggregate([
        {
          $match: {
            date: { $gte: date_debut, $lt: date_fin },
  ...
  async get_nb_benev_session_accueil(
    ...
    const result = await evenement_benev_x_benev
      .aggregate([
        {
          $match: {
            statut: { $eq: 'Présent' },
  ...
  async getViewData(date_debut, date_fin) {
    ...
    const cdpenrbenev = connection.collection('cdpenrbenevs');
    const evenement_benev = connection.collection('evenementbenevs');
    const evenement_benev_x_benev = connection.collection(
      'evenementbenevxbenevs',
    );
    let result = {
      label: 'Pôle Bénévole',
      nb_session_acc: 0,
    ...
    const nb_session_acc = await this.get_nb_session_acc(
      evenement_benev,
      date_debut,
      date_fin,
    );
    const nb_benev_session_accueil = await this.get_nb_benev_session_accueil(
      evenement_benev_x_benev,
      date_debut,
      date_fin,
    );
    const nb_benev_inscrit_session_accueil =
      await this.get_nb_benev_inscrit_session_accueil(
        evenement_benev_x_benev,
        date_debut,
        date_fin,
      );
    const nb_sensi = await this.get_nb_sensi(
      evenement_benev,
      date_debut,
      date_fin,
    );
    const nb_present_sensi = await this.get_nb_present_sensi(
      evenement_benev,
      date_debut,
      date_fin,
    );
    const nb_presence_atelier_benev = await this.get_nb_presence_atelier_benev(
      cdpenrbenev,
      date_debut,
      date_fin,
    );
    const nb_presence_atelier_benev_93_95 =
      await this.get_nb_presence_atelier_benev_93_95(
        cdpenrbenev,
        date_debut,
        date_fin,
      );
    const nb_repartition = await this.get_nb_repartition(
      cdpenrbenev,
      date_debut,
      date_fin,
    );
    const nb_ateliers_moyen = await this.get_nb_atelier_moyen(
      cdpenrbenev,
      date_debut,
      date_fin,
    );
    const nb_formation = await this.get_nb_formation(
      evenement_benev,
      date_debut,
      date_fin,
    );
    ...
    result.benev_en_atelier =
      nb_benev_session_accueil?.avec_date_premier_atelier || 0;
    result.nb_inscrits_session_acc =
      nb_benev_inscrit_session_accueil?.total || 0;
