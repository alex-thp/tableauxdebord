...
export class StatsVetementService {
  ...
  async get_tmp_nb_collecte(evenement_pc, date_debut, date_fin) {
    let tmp_nb_collecte = await evenement_pc
      .aggregate([
        {
          $match: {
  ...
  async get_tmp_nb_fresque(evenement_pc, date_debut, date_fin) {
    let tmp_nb_fresque = await evenement_pc
      .aggregate([
        {
          $match: {
            date: { $gte: date_debut, $lt: date_fin },
  ...
  async get_tmp_nb_tri(evenement_pc, date_debut, date_fin) {
    let tmp_nb_tri = await evenement_pc
      .aggregate([
        {
          $match: {
            date: { $gte: date_debut, $lt: date_fin },
  ...
  async get_nb_benevole_unique_ancien(cdpenrbenev, startOfWeek, endOfWeek) {
    let nbBenevolesUniquesAncien_semaine = await cdpenrbenev
      .aggregate([
        {
          $match: {
            benevole_id: { $ne: null },
  ...
  async get_nb_benevole_unique_semaine(cdpenrbenev, startOfWeek, endOfWeek) {
    let nbBenevolesUniques_semaine = await cdpenrbenev
      .aggregate([
        {
          $match: {
            benevole_id: { $ne: null },
  ...
  async getMobilCollab(cdpenrbenev, start, end) {
    let nbBenevolesByMonth = await cdpenrbenev
      .aggregate([
        {
          $match: {
            benevole_id: { $ne: null },
  ...
  async getMainData(date_debut, date_fin) {
    ...
    let tmp_nb_collecte = await this.get_tmp_nb_collecte(
      evenement_pc,
      date_debut,
      date_fin,
    );

    let tmp_nb_fresque = await this.get_tmp_nb_fresque(
      evenement_pc,
      date_debut,
      date_fin,
    );

    let tmp_nb_tri = await this.get_tmp_nb_tri(
      evenement_pc,
      date_debut,
      date_fin,
    );
    ...
    const nbBenevolesUniquesAncien_semaine =
      await this.get_nb_benevole_unique_ancien(
        cdpenrbenev,
        startOfWeek,
        endOfWeek,
      );

    const nbBenevolesUniques_semaine = await this.get_nb_benevoles_unique(
      cdpenrbenev,
      startOfWeek,
      endOfWeek,
    );
  ...
  async getViewData(date_debut, date_fin) {
    ...
    const nb_venues_ateliers = await this.get_nb_repartition(
      cdpenrbenev,
      date_debut,
      date_fin,
    );
    const nb_collecte = await this.get_tmp_nb_collecte(
      evenement_pc,
      date_debut,
      date_fin,
    );
    const nb_tri = await this.get_tmp_nb_tri(
      evenement_pc,
      date_debut,
      date_fin,
    );
    const nb_fresque = await this.get_tmp_nb_fresque(
      evenement_pc,
      date_debut,
      date_fin,
    );
    const nb_benevoles = await this.get_nb_benevoles_unique(
      cdpenrbenev,
      date_debut,
      date_fin,
    );
    ...
    result.nb_atelier_moyen_par_be = await this.get_nb_atelier_moyen(
      cdpenrbenev,
      date_debut,
      date_fin,
    );
    result.nbCollabsByMonth = await this.getMobilCollab(
      cdpenrbenev,
      date_debut_collab,
      date_fin_collab,
    );
    result.nbCollabsByMonthUnique = await this.getMobilCollabUniques(
      cdpenrbenev,
      date_debut_collab,
      date_fin_collab,
    );
    result.nbNouveauCollab = await this.get_nb_benevole_unique_ancien(
      cdpenrbenev,
      date_debut,
      date_fin,
    );
