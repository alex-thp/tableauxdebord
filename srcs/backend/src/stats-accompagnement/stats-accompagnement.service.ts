...
export class StatsAccompagnementService {
    ...
    async getViewData(date_debut, date_fin) {
        ...
        //const top_structures_par_statut = await this.get_top_structures_par_statut(cdpenrcand, date_debut, date_fin);
        ...
        /*const assoPartString = top_structures_par_statut.map(item => {
          const statut = item.statut;
          const structures = item.topStructures.map((structure, index) => {
              return `${index + 1}. ${structure.structure_id} : ${structure.count}`;
          }).join('\n');
              
          return `Statut: ${statut}\n\n${structures}\n`;  // Ajout de sauts de ligne avant et après les structures
