...
export class PdfMakerController {
  ...
  async generatePdf(@Body('html') html: string, @Res() res: Response) {
    try {
      ...
      // ------------------ Images fixes ------------------
      const images = {
        fondPage: this.toBase64DataUrl(
          path.join(process.cwd(), 'uploads/RA_benevole/Fond_page/fond_1.png'),
          'image/png',
        ),
        persoOrdi: this.toBase64DataUrl(
          path.join(
            process.cwd(),
            'uploads/RA_benevole/Images_PNG/personnage_ordi_haut_de_page.png',
          ),
          'image/png',
        ),
        route: this.toBase64DataUrl(
          path.join(
            process.cwd(),
            'uploads/RA_benevole/Images_PNG/ROUTE_1.png',
          ),
          'image/png',
        ),
        persoDepart: this.toBase64DataUrl(
          path.join(
            process.cwd(),
            'uploads/RA_benevole/Images_PNG/personnage_depart.png',
          ),
          'image/png',
        ),
        persoRh: this.toBase64DataUrl(
          path.join(
            process.cwd(),
            'uploads/RA_benevole/Images_PNG/personnage_rh_2.png',
          ),
          'image/png',
        ),
        verbatim1: this.toBase64DataUrl(
          path.join(
            process.cwd(),
            'uploads/RA_benevole/Images_PNG/Verbatim_1.png',
          ),
          'image/png',
        ),
        verbatim2: this.toBase64DataUrl(
          path.join(
            process.cwd(),
            'uploads/RA_benevole/Images_PNG/Verbatim_2.png',
          ),
          'image/png',
        ),
        persoCoteVeste: this.toBase64DataUrl(
          path.join(
            process.cwd(),
            'uploads/RA_benevole/Images_PNG/personnage_cote_veste.png',
          ),
          'image/png',
        ),
        panneau: this.toBase64DataUrl(
          path.join(
            process.cwd(),
            'uploads/RA_benevole/Images_PNG/panneau.png',
          ),
          'image/png',
        ),
        herbe: this.toBase64DataUrl(
          path.join(process.cwd(), 'uploads/RA_benevole/Images_PNG/herbe.png'),
          'image/png',
        ),
        fleur: this.toBase64DataUrl(
          path.join(process.cwd(), 'uploads/RA_benevole/Images_PNG/fleur.png'),
          'image/png',
        ),
        fleur2: this.toBase64DataUrl(
          path.join(
            process.cwd(),
            'uploads/RA_benevole/Images_PNG/fleur_2.png',
          ),
          'image/png',
        ),
        papillon: this.toBase64DataUrl(
          path.join(
            process.cwd(),
            'uploads/RA_benevole/Images_PNG/papillons.png',
          ),
          'image/png',
        ),
        mapImg: this.toBase64DataUrl(
          path.join(process.cwd(), 'uploads/RA_benevole/Images_PNG/map.png'),
          'image/png',
        ),
        un: this.toBase64DataUrl(
          path.join(process.cwd(), 'uploads/RA_benevole/Images_PNG/1.png'),
          'image/png',
        ),
        deux: this.toBase64DataUrl(
          path.join(process.cwd(), 'uploads/RA_benevole/Images_PNG/2.png'),
          'image/png',
        ),
        trois: this.toBase64DataUrl(
          path.join(process.cwd(), 'uploads/RA_benevole/Images_PNG/3.png'),
          'image/png',
        ),
        photoAction: this.toBase64DataUrl(
          path.join(
            process.cwd(),
            'uploads/RA_benevole/Images_PNG/photo_action.png',
          ),
          'image/png',
        ),
      };
      ...
      // ------------------ Candidat et photo ------------------
      const { photoPath: photoCandidatPath, candidat: candidatCourant } =
        await this.getPhotoCandidat(this.retoursAtelier, this.candidatIndex);

      if (!candidatCourant)
        return res.status(404).send('Aucun candidat trouvé');

      // Incrémenter l'index pour le prochain PDF
      this.candidatIndex =
        (this.candidatIndex + 1) % this.retoursAtelier.length;

      const photoCandidatBase64 = photoCandidatPath
        ? fs.readFileSync(photoCandidatPath, { encoding: 'base64' })
        : '';
      ...
      // ------------------ Génération PDF ------------------
      const page1Pdf: Buffer = await this.pdfMakerService.generatePdfFromHtml(
        htmlContent,
        0,
      );
      const pdfPath = path.join(
        process.cwd(),
        'uploads/RA_benevole/LCS_RA.pdf',
      );
      const pdf2Buffer = fs.readFileSync(pdfPath);
      const finalPdf = await this.pdfMakerService.mergePdfAtPosition(
        pdf2Buffer,
        page1Pdf,
        29,
      );
  ...
  async mergePdf(
    ...
    try {
      ...
      let pdfBuffer = await this.pdfMakerService.mergePdfSimple(
        fileOriginal.buffer,
        fileInsert.buffer,
      );
      res.set({
      ...
      pdfBuffer = await this.pdfMakerService.mergePdfAtPosition(
        pdf2Buffer,
        pdfBuffer,
        29,
      );
