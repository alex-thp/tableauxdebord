...
export class VisualisationComponent {
  ...
  numericFilters: {
    [key: string]: { greaterThan?: number; lessThan?: number };
  } = {};
  ...
  constructor(
    ...
    this.route.paramMap.subscribe((params) => {
      ...
      console.log(
        "Récupération de l'ID du rapport X Indicateur:",
        this.rapportXIndicateurId
      );
      this.devGatewayService
        .getRapportXIndicateur(this.rapportXIndicateurId)
        .subscribe({
          next: (data) => {
  ...
  filterData() {
    this.filteredData = this.dataToDisplay.filter((row) => {
      const matchFilters = Object.entries(this.activeFilters).every(
        ([key, values]) => values.includes(this.formatValue(row[key]))
      );

      const matchDateFilters = Object.entries(this.dateFilters).every(
        ([key, { before, after }]) => {
      ...
      const matchNumericFilters = Object.entries(this.numericFilters).every(
        ([key, { greaterThan, lessThan }]) => {
          const value = row[key];
      ...
      const matchTextFilters = Object.entries(this.textFilters || {}).every(
        ([key, items]) => {
          ...
          return filledValues.some((v) =>
            cellValue.includes(v.toLowerCase().trim())
          );
        }
      );
  ...
  setNumericFilter(
    ...
    if (
      !this.numericFilters[column].greaterThan &&
      !this.numericFilters[column].lessThan
    ) {
  ...
  getUniqueValuesForColumn(column: string): any[] {
    ...
    if (sample instanceof Date) {
      sortedValues = [
        ...new Set(
          nonNullValues.map((v) => (v instanceof Date ? v : new Date(v)))
        ),
      ];
