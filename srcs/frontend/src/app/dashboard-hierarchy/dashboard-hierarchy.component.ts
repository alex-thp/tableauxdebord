...
export class DashboardHierarchyComponent {
  ...
  ngOnInit(): void {
    this.gatewayService.getdashboardData(this.today).subscribe((data) => {
      this.data = data;
      console.log(
        'Données reçues dans DashboardHierarchyComponent:',
        this.data
      );
    });
  }
  ...
  getFilteredData(): any[] {
    return (
      this.data
        ...
        .filter(
          (loc) =>
            this.filtreLocalites.size === 0 ||
            this.filtreLocalites.has(loc.label)
        )
  ...
  private filterActions(actions: any[]): any[] {
    return (
      actions
        ...
        .filter(
          (action) =>
            this.filtreActions.size === 0 ||
            this.filtreActions.has(action.label)
        )

        // 2. Pour chaque action retenue, filtrer ses sous-sujets (niveau 4)
        .map((action) => ({
          ...action,
          next: action.next.filter(
            (su: { label: string }) =>
              this.filtreSujets.size === 0 || this.filtreSujets.has(su.label)
          ),
        }))
  ...
  toggleSelection(
    type: 'localites' | 'actions' | 'sujets',
    value: string,
    checked: boolean
  ) {
    const set =
      type === 'localites'
        ? this.filtreLocalites
        : type === 'actions'
        ? this.filtreActions
        : this.filtreSujets;
        ...
        : Array.from(
            new Set(
              this.data
                .flatMap((d) => d.next)
                .flatMap((s: any) => s.next)
                .flatMap((a: any) => a.next.map((su: any) => su.label))
            )
          );
    const search = (
      type === 'localites'
